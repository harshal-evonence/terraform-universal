# Provision Service Accounts used by GKE, GCE, and DB clients
...
resource "google_service_account" "gke_client_sa" {
  depends_on   = [module.enable_apis]
  account_id   = "${var.environment}-gke-client-sa"
  display_name = "GKE Client Service Account for ${var.environment}"
  project      = var.project_id
  description  = "Service account for GKE client VM in ${var.environment} environment"
}

resource "google_service_account" "db_client_sa" {
  depends_on   = [module.enable_apis]
  account_id   = "${var.environment}-db-client-sa"
  display_name = "Database Client Service Account for ${var.environment}"
  project      = var.project_id
  description  = "Service account for database client VM in ${var.environment} environment"
}
...
module "projects_iam_bindings" {
  depends_on = [
    ...
    google_service_account.gke_client_sa,
  ...
  bindings = {
    ...
    "roles/container.developer" = [
      "serviceAccount:${google_service_account.gke_client_sa.email}",
    ],
